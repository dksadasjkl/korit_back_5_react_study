...
function PhotoRegister() {
    ...
    const handleSubmitCilck = () => {
        const IsSave = window.confirm("사진을 저장하겠습니까?");
        if(!IsSave) {
            return;
        }
        ...
        const lastId = localStorageFiles.length === 0 ? 0 : localStorageFiles[localStorageFiles.length - 1];
    ...
    return (
        <div css={S.layout}>
            <div css={S.header}>
                ...
                <button css={S.submitButton} onClick={handleSubmitCilck}>완료</button>
            </div>
            <input 
                type="file" 
                style={{display: "none"}} 
            ...
            />
            {/* htmlFor(label) === id(input) htmlFor 클릭 시 id가 클릭 */}
            <div css={S.container}>
                {loadPhotos.map(
                    photo => 
                        <div key={photo.id}>
